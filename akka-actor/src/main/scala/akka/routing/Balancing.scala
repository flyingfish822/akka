...
final case class BalancingPool(
  ...
  override private[akka] def newRoutee(routeeProps: Props, context: ActorContext): Routee = {
    ...
    val dispatcherId = s"akka.actor.deployment.$deployPath.pool-dispatcher"
    ...
    if (!dispatchers.hasDispatcher(dispatcherId)) {
      ...
      val dispatcherConfig = context.system.dispatchers.config(dispatcherId, ConfigFactory.parseString(
        s"""$dispatcherId.type = "${classOf[BalancingDispatcherConfigurator].getName}" """).
        withFallback(
          // use the user defined 'pool-dispatcher' config as fallback, if any   
          if (routeeProps.deploy.config.hasPath(dispatcherId)) routeeProps.deploy.config.getConfig(dispatcherId)
