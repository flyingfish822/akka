...
class RemoteConfigSpec extends AkkaSpec(
  ...
  "Remoting" must {
    ...
    "contain correct configuration values in reference.conf" in {
      ...
      RetryGateClosedFor must be(5 seconds)
      Dispatcher must be === "akka.remote.default-remote-dispatcher"
      UsePassiveConnections must be(true)
      BackoffPeriod must be(10 millis)
      SysMsgAckTimeout must be(0.3 seconds)
      SysResendTimeout must be(2 seconds)
      SysMsgBufferSize must be(1000)
      QuarantineDuration must be(5 days)
      ...
      Duration(WatchFailureDetectorConfig.getMilliseconds("acceptable-heartbeat-pause"), MILLISECONDS) must be(10 seconds)
    ...
    "be able to parse AkkaProtocol related config elements" in {
      ...
      TransportHeartBeatInterval must be === 3.seconds
      ...
      Duration(TransportFailureDetectorConfig.getMilliseconds("acceptable-heartbeat-pause"), MILLISECONDS) must be(20 seconds)
